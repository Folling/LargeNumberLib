...
public:
	...
	char getSign()               const;
	void setSign(char newSign) const;
	void setValue(const std::vector<int>& input);
	void append(const largeInt& tail);
	uint size() const;
	void resize(int newSize, int filler) const;
	void popFront();
	void popEnd();
	void push_back(int tail) const;
public:
...
public:
	...
	largeInt operator  % (const largeInt& divisor) const;
public:
	template<typename T> largeInt operator +(const T summand) const;
	template<typename T> largeInt operator -(const T subtrahend) const;
	template<typename T> largeInt operator *(const T factor) const;
	template<typename T> largeInt operator /(const T divisor) const;
	template<typename T> largeInt operator %(const T divisor) const;
public:
	...
	largeInt operator %= (const largeInt& divisor);
public:
	template<typename T> largeInt& operator = (const T setTo);
	template<typename T> largeInt operator += (const T summand);
	template<typename T> largeInt operator -= (const T subtrahend);
	template<typename T> largeInt operator *= (const T factor);
	template<typename T> largeInt operator /= (const T divisor);
	template<typename T> largeInt operator %= (const T divisor);
public:
...
public:
	...
	bool operator !  () const;
public:
	template<typename T> bool operator == (const T test) const;
	template<typename T> bool operator >  (const T test) const;
	template<typename T> bool operator <  (const T test) const;
	template<typename T> bool operator >= (const T test) const;
	template<typename T> bool operator <= (const T test) const;
